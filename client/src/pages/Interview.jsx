...
export default function Interview() {
  ...
  return (
    <div className="h-screen overflow-hidden bg-slate-50 dark:bg-[#0B0F19] text-slate-800 dark:text-white flex flex-col font-sans transition-colors duration-300">
      ...
      {/* Top Navbar */}
      <div className="h-16 border-b border-slate-200 dark:border-gray-800 px-3 md:px-6 flex items-center justify-between bg-white dark:bg-[#0B0F19] transition-colors duration-300 gap-2">
        <div className="flex items-center gap-2 md:gap-3 min-w-0">
          <div className="w-8 h-8 md:w-10 md:h-10 bg-indigo-500 rounded-lg md:rounded-xl flex items-center justify-center shadow-md shrink-0">
            <Bot className="w-5 h-5 md:w-6 md:h-6 text-white" />
          </div>
          <div className="min-w-0">
            <h1 className="text-sm md:text-base font-bold text-slate-900 dark:text-white truncate">{config.position}</h1>
            <p className="text-[10px] md:text-xs text-slate-500 dark:text-gray-400 truncate">AI Interview</p>
          </div>
        </div>

        <div className="text-xs md:text-sm font-semibold text-slate-600 dark:text-gray-400 shrink-0">
          <span className="hidden md:inline">Question </span>{questionIndex}/{totalQuestions}
        </div>

        <div className="flex items-center gap-2 md:gap-4 shrink-0">
          <div className="flex items-center gap-1.5 md:gap-2 border border-slate-200 dark:border-gray-700 rounded-full px-3 md:px-4 py-1 md:py-1.5 bg-slate-50 dark:bg-[#111423]">
            <Clock className="w-3 h-3 md:w-4 md:h-4 text-slate-500 dark:text-gray-400" />
            <span className="text-xs md:text-sm font-semibold text-slate-800 dark:text-white">{formatTime(timer)}</span>
          </div>
          <button 
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
        ...
        <div className="w-full h-[60%] md:h-auto md:w-1/2 flex flex-col border-b md:border-b-0 md:border-r border-slate-200 dark:border-gray-800 bg-slate-50 dark:bg-[#0B0F19]">
          
          {/* Fixed Top Section: AI Recruiter Question Box */}
          <div className="p-3 md:p-6 border-b border-slate-200 dark:border-gray-800 shrink-0 bg-white dark:bg-[#0B0F19] z-10 shadow-xs relative">
            <div className="flex items-start gap-3 md:gap-4">
              <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-blue-50 dark:bg-indigo-900/30 border border-blue-100 dark:border-indigo-800/50 flex items-center justify-center shrink-0 shadow-sm mt-1">
                <Bot className="w-4 h-4 md:w-5 md:h-5 text-blue-600 dark:text-indigo-400" />
              </div>
              <div className="bg-white dark:bg-[#131722] rounded-xl md:rounded-2xl p-3 md:p-5 border border-slate-200 dark:border-gray-800/60 shadow-lg w-full relative flex flex-col max-h-[20vh] md:max-h-[30vh]">
        ...
        <div className="w-full h-[40%] md:h-auto md:w-1/2 p-3 md:p-8 flex flex-col gap-3 md:gap-4 bg-slate-50 dark:bg-[#0B0F19]">
          <div className="hidden md:flex bg-white dark:bg-[#151924] border border-slate-200 dark:border-gray-800 rounded-xl p-4 items-center gap-2 shadow-sm">
          ...
          <div className="flex-1 flex flex-col bg-white dark:bg-[#111423] border border-slate-200 dark:border-gray-800 rounded-xl overflow-hidden shadow-inner focus-within:border-indigo-500/50 dark:focus-within:border-indigo-500/50 transition-colors">
            <textarea
              className="flex-1 w-full bg-transparent p-4 md:p-6 outline-none text-slate-800 dark:text-gray-200 resize-none custom-scrollbar text-base md:text-lg placeholder-slate-400 dark:placeholder-gray-600"
          ...
          <button
            ...
            className="w-full py-3 md:py-4 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-semibold flex items-center justify-center gap-2 transition-all disabled:opacity-50 disabled:hover:bg-indigo-600 shadow-md cursor-pointer"
          >
